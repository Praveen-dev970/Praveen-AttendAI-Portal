...
import { AttendanceAnalyticsWidgets } from './attendance/AttendanceAnalyticsWidgets';
...
export default function AttendanceView({
  ...
  const filteredRecords = records
    .filter((rec) => {
      const matchesSubject =
        filterSubject === 'all' || rec.subjectId === filterSubject;
      const matchesStatus =
        filterStatus === 'all' || rec.status === filterStatus;

      const subjectName =
        subjects.find((s) => s.subject === rec.subjectId)?.subject || '';
      const matchesSearch =
  ...
  return (
    <div className="space-y-6">
      ...
      <AttendanceAnalyticsWidgets />

      {/* Existing page layout (manual form + archive) */}
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 select-none">
          ...
          <div className={`${APP_CARD} p-5 md:p-6`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              >
                ...
                <span>
                  {isSubmitting ? 'Logging Record...' : 'Record Lecture'}
                </span>
              </button>
            </form>
          </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          <div
            className={`${APP_CARD} flex h-full min-h-[500px] flex-col justify-between p-5 md:p-6`}
          >
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 py-4 text-xs">
                ...
                >
                  ...
                  <option value="present" className="dark:bg-slate-900">
                    Present
                  </option>
                  <option value="late" className="dark:bg-slate-900">
                    Late
                  </option>
                  <option value="absent" className="dark:bg-slate-900">
                    Absent
                  </option>
                </select>
              </div>
              ...
              {filteredRecords.length === 0 ? (
                <div className="flex min-h-[280px] flex-col items-center justify-center rounded-2xl border border-dashed border-slate-200/80 bg-slate-50/50 p-6 text-center text-slate-400 dark:border-slate-800 dark:bg-slate-900/20 dark:text-slate-500">
                  ...
                  <p className="text-xs font-display font-medium">
                    No recorded logs match the current filters.
                  </p>
                </div>
              ) : (
                  ...
                  {filteredRecords.map((rec) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <span
                            className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-md border tracking-wider uppercase ${
                              rec.status === 'present'
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-800/80 text-[10px] text-slate-400 dark:text-slate-500 flex items-center justify-between font-mono">
              <span>
                Showing {filteredRecords.length} of {records.length} database logs
              </span>
